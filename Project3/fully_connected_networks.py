...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        z = x.view(x.shape[0], -1).clone() # shape: (N, D)
        out = torch.mm(z,w) + b.t()
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * (torch.pow(self.params['W1'],2).sum() +torch.pow(self.params['W2'],2).sum())
        
        dx, dw, db = Linear.backward(dout, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        output_dim = hidden_dims + [num_classes] 
        w_dim = input_dim
        for i in range(self.num_layers):
            self.params[f'W{i+1}'] = torch.randn(w_dim, output_dim[i], dtype=dtype,device= device) * weight_scale
            self.params[f'b{i+1}'] = torch.zeros(output_dim[i], dtype=dtype,device= device)
            w_dim = output_dim[i]
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            out, cache[i+1] = Linear_ReLU.forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
            if self.use_dropout:
                out, cache[f'{i+1}_d'] = Dropout.forward(out, self.dropout_param)
        i += 1
        scores, cache[i+1] = Linear.forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        loss, dout = softmax_loss(scores, y)
        for i in range(self.num_layers):
            loss += self.reg * (torch.pow(self.params[f'W{i+1}'],2)).sum() 
        
        last_layer = self.num_layers
        dx, dw, db = Linear.backward(dout, cache[last_layer])
        grads[f'W{last_layer}'] = dw + 2*self.reg*self.params[f'W{last_layer}'] 
        grads[f'b{last_layer}'] = db
        
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dx = Dropout.backward(dx, cache[f'{i}_d'])
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad_sq = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * dw * dw
    config['cache'] = grad_sq
    next_w = w - config['learning_rate'] * dw / (grad_sq.sqrt() + 1e-7)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    m1_bias = config['m'] / (1 - config["beta1"]** config['t'])
    m2_bias = config['v'] / (1 - config["beta2"]** config['t'])
    next_w = w - (config['learning_rate'] * m1_bias) / (m2_bias.sqrt() + config['epsilon'])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(x.shape, device = x.device, dtype = x.dtype) < 1 - p) / (1-p)
            out = x*mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
