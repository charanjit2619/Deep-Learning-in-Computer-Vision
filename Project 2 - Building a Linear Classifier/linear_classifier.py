...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    scores = X.mm(W)
    correct_scores = scores[np.arange(scores.shape[0]), y.reshape(-1)].reshape((X.shape[0], 1))
    margin = scores - correct_scores + 1
    margin[torch.arange(scores.shape[0]), y.reshape(-1)] = 0 # putting margin at correct classes = 0
    loss = torch.mean(torch.sum(torch.max(margin, torch.zeros_like(margin)), dim = 1, keepdim = True)) + reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    margin_bool = (margin > 0).float()
    margin_bool[np.arange(margin_bool.shape[0]), y.reshape(-1)] -= margin_bool.sum(dim = 1)
    dW = X.t().double().mm(margin_bool.double())
    dW = dW/X.shape[0]
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def softmax_loss_naive(
    ...
    for i in range(num_train): 
        ...
        dW_b = X[i].reshape(-1, 1).repeat(1, num_classes) 
        dW_b *= scores[i].reshape(1, -1)
        dW_b[:,y[i]] += -X[i]
        dW+= dW_b 
...
def softmax_loss_vectorized(
    ...
    loss = torch.sum(-torch.log(scores[[idx0, y]]))
    loss /= num_train
    loss += reg * torch.sum(W * W) 
      
    dW_pos = X.t() @ scores
    dW_neg = torch.zeros_like(scores)
    dW_neg[[idx0, y]] = 1 
    dW_neg = -X.t()@dW_neg
