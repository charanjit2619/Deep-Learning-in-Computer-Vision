...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.max(X.mm(W1) + b1, torch.zeros_like(X.mm(W1)))
...
def nn_forward_backward(
    ...
    exp_scores = torch.exp(scores - max_score.view(-1, 1))
    predictions = (exp_scores / torch.sum(exp_scores, dim = 1).view(-1, 1))
    loss = torch.mean(torch.log(predictions[np.arange(scores.shape[0]), y.reshape(-1)])) * (-1) + reg * (torch.sum(W1*W1) + torch.sum(W2*W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # Replace "pass" statement with your code
    dloss = 1/N
    dsoftmax = predictions*dloss
    dsoftmax[np.arange(predictions.shape[0]), y.reshape(-1)] -= 1/N
    grads['b2'] = dsoftmax.sum(dim = 0)
    grads['W2'] = (h1.t()).mm(dsoftmax) + 2*reg*W2
    dh1 = dsoftmax.mm(W2.t())
    drelu = torch.mul(torch.where(h1 > 0, torch.ones_like(dh1), torch.zeros_like(dh1)), dh1)
    grads['b1'] = drelu.sum(dim = 0)
    grads['W1'] = (X.t()).mm(drelu) + 2*reg*W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    max_scores, y_pred = torch.max(scores, dim = 1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.1, 1.401]
    hidden_sizes = [130, 500]
    regularization_strengths = [0.001, 0]
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for decay in learning_rate_decays:
                    ...
                    trained_net_stats = net.train(X_train, y_train, X_val, y_val, num_iters = num_iters, batch_size = batch_size,
                                        learning_rate=lr, learning_rate_decay=0.95,
                                        reg=reg, verbose=False)
                    if trained_net_stats['val_acc_history'].pop() > best_val_acc:
                        best_net = net
                        best_stat = trained_net_stats
                        best_val_acc = trained_net_stats['val_acc_history'].pop()
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
