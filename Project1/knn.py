...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = (x_train[i].view(-1) - x_test[j].view(-1)).pow(2).sum(axis = 0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        dists[i, :] = (x_train[i].view(-1) - x_test.reshape(num_test, -1)).pow(2).sum(axis = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.clone().view(num_train, -1) 
    x_test_flat = x_test.clone().view(num_test, -1)    

    x_train_norms = torch.sum(x_train_flat ** 2, dim=1, keepdim=True) 
    x_test_norms = torch.sum(x_test_flat ** 2, dim=1, keepdim=True)
    dists = x_train_norms - 2 * torch.mm(x_train_flat, x_test_flat.T) + x_test_norms.T    
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            accuracy = classifier.check_accuracy(test_data, test_labels, k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, acc_list in k_to_accuracies.items():
        k_values.append(k)
        mean.append(sum(acc_list)/len(acc_list))
    best_k = k_values[mean.index(max(mean))]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
