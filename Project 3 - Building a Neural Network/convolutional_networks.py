...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        l = (W + 2 * pad - WW) // stride + 1

        out = torch.zeros((N, F, h, l),device = x.device, dtype = x.dtype)
            ...
            for f in range(F):
                for i in range(h):
                    for j in range(l):
                        ...
                        input_slice = x_padded[n, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
                        

                        # Perform convolution at each location
                        out[n, f, i, j] = (input_slice * w[f]).sum() + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
...
class MaxPool(object):
    ...
    def backward(dout, cache):
        ...
        dx = torch.zeros_like(x)  # Initialize the gradient dx
        
        for n in range(N):
            for c in range(C):
                for h in range(H_out):
                    for w in range(W_out):
                        # Find the indices of the maximum value in the forward pass
                        h_start, h_end = h * stride, h * stride + pool_height
                        w_start, w_end = w * stride, w * stride + pool_width
                        x_slice = x[n, c, h_start:h_end, w_start:w_end]
                        max_idx = torch.argmax(x_slice)
                        max_idx = divmod(max_idx.item(), pool_width)  # Convert 1D index to 2D
                        
                        # Accumulate the gradient to the corresponding input region
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = torch.randn(num_filters*H_out*W_out, hidden_dim, device = device, dtype = dtype) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        cache= {}
        scores, cache['1'] = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        scores, cache['2'] = Linear_ReLU.forward(scores, W2, b2)
        scores , cache['3'] = Linear.forward(scores, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dx, grads['W3'], grads['b3'] = Linear.backward(dout, cache['3'])
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers - 1):
          if weight_scale != "kaiming":
            self.params[f'W{i+1}'] = torch.randn(num_filters[i], C, filter_size, filter_size, device = device, dtype = dtype) * weight_scale
          else:
            self.params[f'W{i+1}'] = kaiming_initializer(C, num_filters[i], K=filter_size, relu=True, device=device,dtype=dtype)
        ...
        if weight_scale != "kaiming":
          self.params[f'W{i+2}'] = torch.randn(num_filters[i]*H_out*W_out, num_classes, device = device, dtype = dtype) * weight_scale
        else:
          self.params[f'W{i+2}'] = kaiming_initializer(num_filters[i]*H_out*W_out, num_classes, K=None, relu=False, device=device,dtype=dtype)
        self.params[f'b{i+2}'] = torch.zeros(num_classes, device = device, dtype = dtype)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          if i in self.max_pools:
            if self.batchnorm:
              scores, cache[f'{i+1}'] = Conv_BatchNorm_ReLU_Pool.forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'],
              self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], conv_param, self.bn_params[i], pool_param) 
            else:  
              scores, cache[f'{i+1}'] = Conv_ReLU_Pool.forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'], conv_param, pool_param)
          else:
            if self.batchnorm:
              scores, cache[f'{i+1}'] = Conv_BatchNorm_ReLU.forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'],
              self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], conv_param, self.bn_params[i])
            else:
              scores, cache[f'{i+1}'] = Conv_ReLU.forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'], conv_param)
        scores, cache[f'{i+2}'] = Linear.forward(scores, self.params[f'W{i+2}'], self.params[f'b{i+2}'])
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        dx, grads[f'W{i}'], grads[f'b{i}'] = Linear.backward(dout, cache[f'{i}'])
        ...
        for i in range(self.num_layers - 2, -1, -1):
          if i in self.max_pools:
            if self.batchnorm:
              dx, grads[f'W{i+1}'], grads[f'b{i+1}'], grads[f'gamma{i+1}'], grads[f'beta{i+1}']  = Conv_BatchNorm_ReLU_Pool.backward(dx, cache[f'{i+1}'])
              grads[f'W{i+1}'] += 2*self.params[f'W{i+1}']*self.reg
              # grads[f'gamma{i+1}'] += 2*self.params[f'gamma{i+1}']*self.reg
            else:
              dx, grads[f'W{i+1}'], grads[f'b{i+1}'] = Conv_ReLU_Pool.backward(dx, cache[f'{i+1}'])
              grads[f'W{i+1}'] +=  2*self.reg*self.params[f'W{i+1}']    
          else:
            if self.batchnorm:
              dx, grads[f'W{i+1}'], grads[f'b{i+1}'], grads[f'gamma{i+1}'], grads[f'beta{i+1}']  = Conv_BatchNorm_ReLU.backward(dx, cache[f'{i+1}'])
              grads[f'W{i+1}'] += 2*self.params[f'W{i+1}']*self.reg
              # grads[f'gamma{i+1}'] += 2*self.params[f'gamma{i+1}']*self.reg
            else:
              dx, grads[f'W{i+1}'], grads[f'b{i+1}'] = Conv_ReLU.backward(dx, cache[f'{i+1}'])
              grads[f'W{i+1}'] +=  2*self.reg*self.params[f'W{i+1}']    
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
class BatchNorm(object):
    ...
    def backward_alt(dout, cache):
        ...
        dx = (1.0 / N) * ivar * (N * dx_normalized - dx_normalized.sum(dim=0) - x_normalized * (dx_normalized * x_normalized).sum(dim=0))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        x_reshaped = x.permute(0, 2, 3, 1).contiguous().reshape(-1, C)
        
        out_reshaped, cache = BatchNorm.forward(x_reshaped, gamma, beta, bn_param)
        
        out = out_reshaped.reshape(N, H, W, C).permute(0, 3, 1, 2).contiguous()
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        # Reshape the upstream gradient
        dout_reshaped = dout.permute(0, 2, 3, 1).contiguous().reshape(-1, C)
        
        # Use vanilla batch normalization backward pass
        dx_reshaped, dgamma, dbeta = BatchNorm.backward(dout_reshaped, cache)
        
        # Reshape the gradient back to the original shape
        dx = dx_reshaped.reshape(N, H, W, C).permute(0, 3, 1, 2).contiguous()
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
